...
QQC2.StackView {
	...
	function getImageData() {
		return new Promise((res, rej) => {
			var url = `https://static.drsmyrke.ru/wallpapers/?`

			var categories = ""
			categories += (wallpaper.configuration.CategoryGeneral) ? "1" : "0";
			categories += (wallpaper.configuration.CategoryMy) ? "1" : "0";
			categories += (wallpaper.configuration.CategoryPhotos) ? "1" : "0";
			url += `cat=${categories}&`
			...
			*/
			...
			xhr.onerror = () => {
				rej("failed to send request ["+url+"]");
			};
			xhr.open('GET', url);
			xhr.setRequestHeader('X-API-Key', wallpaper.configuration.APIKey);
			xhr.setRequestHeader('User-Agent','drsmyrke-wallpaper-kde-plugin');
			xhr.timeout = 15000;
			xhr.send();
		});
	}
	...
	function pickImage(d) {
		if (d.data.length > 0) {
			...
			root.currentUrl = imageObj.url;
			/*root.currentPage = d.meta.current_page;*/
			root.currentPage = 1;
			wallpaper.configuration.currentWallpaperThumbnail = imageObj.thumb.small;
